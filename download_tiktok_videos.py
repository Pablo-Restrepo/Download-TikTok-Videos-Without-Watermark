...
def __get_download_link(url) -> str:
    ...
    try:
        ...
        if response.status_code != 200:
            print(
                f"Failed to load the page. Status code: {response.status_code}")
...
def __download_video(link) -> bytes:
    try:
...
def download_video(url: str) -> str:
    ...
    download_link = __get_download_link(url)
    ...
    video_data = __download_video(download_link)
